...
const ProductDetail = () => {
  ...
  if (isLoading) {
    return (
      <Layout><div className="px-8 py-32 text-center"><p className="text-sm text-muted-foreground">Loading product...</p></div></Layout>
    );
  }

  if (!product) {
    return (
      <Layout><div className="px-8 py-32 text-center">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-8 lg:py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
            <div className="aspect-square rounded-2xl overflow-hidden bg-secondary mb-4 relative">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="flex flex-col">
            ...
            <p className="text-2xl font-medium text-primary mb-2">PKR {product.price.toLocaleString()}</p>
            {outOfStock ? (
              <p className="text-sm text-destructive mb-6">Currently out of stock</p>
            ) : (
              <p className="text-xs text-muted-foreground mb-6">{product.stock_quantity} in stock</p>
            )}
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button onClick={handleOrderNow} disabled={outOfStock}
                    className="flex-1 flex items-center justify-center gap-2 text-sm bg-primary text-primary-foreground rounded-full px-6 py-3.5 hover:bg-gold-glow transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
                    <ShoppingBag className="w-4 h-4" /> {outOfStock ? "Out of Stock" : "Order Now"}
                  </button>
                </TooltipTrigger>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button onClick={handleAddToCart} disabled={outOfStock}
                    className="flex-1 flex items-center justify-center gap-2 text-sm border border-primary/40 text-primary rounded-full px-6 py-3.5 hover:bg-primary hover:text-primary-foreground transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
